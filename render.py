...
def render_set(model_path, name, iteration, views, scene, pipeline, output_video, cam_order, cal_metrics=True):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        t = time.time()
        rendering = torch.clip(render(view, scene.gaussians, scene.env_map, pipeline, scene.photo)["render"], 0.0, 1.0)
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, mode : str, output_video : bool, cam_order : list):
    with torch.no_grad():
        ...
        # filter_3D 不存盘，用训练相机在加载时重算一次（确定性的，和训练时一致）
        gaussians.mip_filter = bool(getattr(dataset, 'mip_filter', False))
        gaussians.compute_3D_filter(scene.getTrainCameras())
        # NB: `mode` is already this function's render-mode argument -- do not shadow it.
        photo_mode = getattr(dataset, 'photo_test_mode', 'spline')
        if scene.photo.enabled and photo_mode != 'spline':
            scene.photo.freeze(photo_mode, [c.time for c in scene.getTrainCameras()])
            print(put_color('[Photometric] test-time mode: ' + photo_mode, color='violet'))
