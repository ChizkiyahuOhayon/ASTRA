    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, env_map: EnvironmentMap, load_iteration=None, shuffle=True, resolution_scales=[1.0]):
        ...
        if os.path.exists(os.path.join(args.source_path, "poses.npz")):
            ...
            scene_info = sceneLoadTypeCallbacks["Waymo"](args.source_path, args.use_colmap, args.num_cam,
                                                        far_init=bool(getattr(args, 'far_init', False)))
        ...
        frame_num = len(scene_info.train_cameras) + len(scene_info.test_cameras)
        # getattr keeps checkpoints trained before this module was added loadable.
        self.photo = PhotometricSpline(getattr(args, 'photo_ctrl_pts', 0), getattr(args, 'photo_order', 3), args.num_cam,
                                       frame_num, args.default_order_downsample_ratio,
                                       center=bool(getattr(args, 'photo_center', 1)))
        if self.photo.enabled:
            print("PhotometricSpline: ctrl_pts =", self.photo.ctrl_pts, "order =", self.photo.order_args[1],
                  "centered =", self.photo.center)
        ...
        if self.loaded_iter:
            ...
            self.gaussians.load_ply(os.path.join(ckpt_path, "point_cloud.ply"),
                                    legacy_shared_motion=getattr(args, 'shared_motion_legacy', ''))
            if getattr(args, 'shared_motion', False) and self.gaussians.shared_motion is None:
                raise FileNotFoundError(os.path.join(ckpt_path, 'shared_motion.pth'))
            self.env_map.load_weights(os.path.join(ckpt_path, "env.pth"))
            self.photo.load_weights(os.path.join(ckpt_path, "photo.pth"))
        else:
            ...
            if getattr(args, 'shared_motion', False):
                self.gaussians.initialize_shared_motion(
                    getattr(args, 'shared_motion_support_gate', False),
                    getattr(args, 'shared_motion_linear_fallback', False),
                    getattr(args, 'shared_motion_piecewise_fallback', False),
                    getattr(args, 'shared_motion_bidirectional', False),
                    getattr(args, 'shared_motion_singleton_static', False),
                    getattr(args, 'shared_motion_rigid_rotation', False),
                    getattr(args, 'shared_motion_registration_init', False),
                    adgs_residual=getattr(args, 'shared_motion_adgs_residual', False),
                    adgs_ungated=getattr(args, 'shared_motion_adgs_ungated', False),
                )
        # Reload restores the route from the checkpoint; the cache is a runtime
        # option, so render.py must re-apply it from cfg_args.
        if self.gaussians.shared_motion is not None:
            self.gaussians.shared_motion.basis_cache = getattr(
                args, 'shared_motion_basis_cache', False)
            self.gaussians.shared_motion.rigid_rotation = getattr(
                args, 'shared_motion_rigid_rotation', False)

    def save(self, iteration):
        point_cloud_path = os.path.join(self.model_path, "point_cloud/iteration_{}".format(iteration))
        self.gaussians.save_ply(os.path.join(point_cloud_path, "point_cloud.ply"))
        self.env_map.save_weights(os.path.join(point_cloud_path, "env.pth"))
        self.photo.save_weights(os.path.join(point_cloud_path, "photo.pth"))
