...
def readWaymoInfo(path, use_colmap=False, num_cam: int = 1, far_init: bool = False):
    ...
    # far_init: 换成补过远端点的点云。原始点云可见 p99 观测距离只有 ~60 m
    # （Waymo 顶置 LiDAR 量程），而 9~12% 的背景像素在 80 m 之外；densification
    # 只能就地分裂，造不出新深度上的几何，所以远端只能靠近处高斯拉长去糊。
    ply_path = os.path.join(path, "points3d-far.ply" if far_init else "points3d.ply")
