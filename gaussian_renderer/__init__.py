...
import torch
import math
# The absolute-gradient rasterizer is a drop-in replacement: identical forward, and an
# extra accumulator in the backward pass that stores sum|d(loss)/d(mean2D)| in the unused
# .z slot of the screen-space gradient. Selected with ADGS_RASTERIZER=abs.
USE_ABS_RASTERIZER = os.environ.get('ADGS_RASTERIZER', '') == 'abs'
if USE_ABS_RASTERIZER:
...
def render(viewpoint_camera, pc : GaussianModel, env_map : EnvironmentMap, pipe, photo : PhotometricSpline = None, scaling_modifier = 1.0, override_color = None, flow_pkg=None, render_objmask=False):
    """
    Render the scene. 
    ...
    # Mip-Splatting 3D 滤波（关闭时逐位等价原版）
    scales = pc.get_scaling_filtered
    opacity = pc.apply_opacity_compensation(opacity)
    ...
    # Time-continuous camera response (identity when disabled).
    if photo is not None:
        rendered_image = photo.apply(rendered_image, viewpoint_camera)
